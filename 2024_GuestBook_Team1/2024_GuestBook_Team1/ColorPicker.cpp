...
void ColorPicker::PaletteControl(POINT mouse_position)
{
    ...
    Gdiplus::Color rgbColor = HSVToRGB(360.0f - h_, s_, 1.0f - v_);
...
void ColorPicker::Draw(HDC hdc)
{
    ...
    Gdiplus::Pen black_pen(Gdiplus::Color(255, 0, 0, 0));                 //팔레트 선택 바깥쪽 윤곽선
    Gdiplus::Pen white_pen(Gdiplus::Color(255, 255, 255, 255));           // 팔레트 선택 안쪽 윤곽선
    Gdiplus::Pen thumb_contour_pen(Gdiplus::Color(255, 149, 149, 149));   //슬라이더 윤곽선
    ...
    Gdiplus::LinearGradientBrush palette_horizontal(
        Gdiplus::Point(palette_x_, palette_y_),
        Gdiplus::Point(palette_x_ + palette_width_, palette_y_),
    ...
    // 팔레트 그라데이션 (위아래: 투명 ~ 검정)
    Gdiplus::LinearGradientBrush palette_vertical(
        Gdiplus::Point(palette_x_, palette_y_),
        Gdiplus::Point(palette_x_, palette_y_ + palette_height_),
    ...
    Gdiplus::Image hue_slider_image(L"../ImgResource/Hue.png");
...
// HSV 값을 RGB 값으로 변환
Gdiplus::Color ColorPicker::HSVToRGB(double h, double s, double v)
{
    double r = 0;
    double g = 0;
    double b = 0;
